...
public class ChapterService {
    ...
    /**
     * 根据章节规划生成章节内容（SSE流式输出）
     *
     * @param projectId 项目ID
     * @param planId 章节规划ID
     * @param userId 用户ID（从Controller传递，避免异步线程中获取不到）
     * @param emitter SSE发射器
     */
    public void generateChapterByPlan(Long projectId, Long planId, Long userId, SseEmitter emitter) {
        CompletableFuture.runAsync(() -> {
            try {
                // 设置UserContext到异步线程，让整个异步执行过程都能访问用户ID
                UserContext.setUserId(userId);

                // 1. 查询章节规划
                NovelChapterPlan plan = chapterPlanMapper.selectById(planId);
                if (plan == null) {
                    sendError(emitter, "章节规划不存在");
                    return;
                }
                ...
                if (chapter == null) {
                    chapter = chapterPersistenceService.createChapterFromPlan(plan);
                    log.info("创建新章节，规划ID: {}, 章节ID: {}", planId, chapter.getId());
                } else {
                    log.info("使用已存在的章节，规划ID: {}, 章节ID: {}", planId, chapter.getId());
                }

                // 2. 查询大纲信息（可能有多条，取最新的一条）
                LambdaQueryWrapper<NovelOutline> outlineQuery = new LambdaQueryWrapper<>();
                outlineQuery.eq(NovelOutline::getProjectId, projectId)
                        .orderByDesc(NovelOutline::getId)
                        .last("LIMIT 1");
                NovelOutline outline = outlineMapper.selectOne(outlineQuery);

                // 3. 查询世界观信息（可能有多条，取最新的一条）
                ...
                // 4. 查询分卷信息
                NovelVolumePlan volume = volumePlanMapper.selectById(plan.getVolumePlanId());

                // 4.5 查询前N章的完整内容（用于剧情连贯性）
                List<Chapter> recentChapters = getRecentChapters(projectId, plan.getChapterNumber(), 5);

                // 4.6 构建章节上下文（数据库优先策略）
                ChapterContext chapterContext = buildChapterContext(projectId, plan);

                // 5. 使用模板系统构建提示词（使用新的上下文）
                String prompt = buildPromptFromTemplate(projectId, plan, volume, worldview, recentChapters, chapterContext);

                // 6. 发送开始事件
                sendEvent(emitter, "start", "开始生成章节内容...");
                ...
                request.setRequestType("llm_chapter_generate_standard");
                request.setRole(AIRole.NOVEL_WRITER);
                request.setTask(prompt);
                // 移除maxTokens限制，让LLM自然完成章节内容生成
                // 不再强制截断，完整接收LLM返回的所有内容
                request.setMaxTokens(null); // null表示不限制
                request.setTemperature(0.8);

                // 获取LLM提供商
                LLMProvider provider = aiGenerateService.getProvider(null);
                ...
                final int[] currentLength = {0}; // 使用数组以便在lambda中修改
                final int[] targetLength = {plan.getWordCountTarget() != null ? plan.getWordCountTarget() : 3000};
                final int[] lastSavedLength = {0}; // 上次保存时的字数
                final int SAVE_INTERVAL = 200; // 每200字保存一次

                // 用于缓冲内容，避免每个token都发送一次SSE
                final var buffer = new StringBuilder();
                final int BUFFER_SIZE = 100; // 减小缓冲区大小，提高实时性
                final boolean[] isStreamComplete = {false};
                final long[] lastTokenTime = {System.currentTimeMillis()};

                // 为了在lambda中使用chapter和其他变量，创建final引用
                final Chapter finalChapter = chapter;
                final String finalTraceId = traceId;
                final long finalStartTime = startTime;
                final String finalPrompt = prompt;
                final Long finalProjectId = projectId;
                final AIGenerateRequest finalRequest = request;

                // 8. 真正的流式生成
                provider.generateStream(request, new LLMProvider.StreamCallback() {
                    @Override
                    public void onText(String text, boolean isDone) {
                        if (!text.isEmpty()) {
                            // 移除字数截断逻辑，允许LLM生成完整内容
                            contentBuilder.append(text);
                            buffer.append(text);
                            currentLength[0] += text.length();
                            lastTokenTime[0] = System.currentTimeMillis(); // 更新最后接收时间

                            // 检查是否需要自动保存（每200字）
                            if (currentLength[0] - lastSavedLength[0] >= SAVE_INTERVAL) {
                                try {
                                    // 保存当前进度到数据库
                                    chapterPersistenceService.appendContent(
                                        finalChapter.getId(),
                                        contentBuilder.toString(),
                                        currentLength[0]
                                    );
                                    lastSavedLength[0] = currentLength[0];

                                    // 发送保存事件通知前端
                                    sendEvent(emitter, "save", Map.of(
                                        "saved", true,
                                        "currentWords", currentLength[0]
                                    ));

                                    log.debug("自动保存章节，章节ID: {}, 当前字数: {}", finalChapter.getId(), currentLength[0]);
                            ...
                            // 当缓冲区达到指定大小，或者遇到换行符时发送
                            // 检查是否包含完整的段落（双换行符）
                            boolean shouldSend = buffer.length() >= BUFFER_SIZE
                                    || buffer.toString().contains("\n\n")
                                    || buffer.toString().contains("\r\n\r\n");

                            if (shouldSend) {
                                try {
                                    // 流式输出时直接发送原始内容，不做格式处理
                                    // 格式处理由前端完成
                                    var toSend = buffer.toString();
                                    buffer.setLength(0);

                                    int progress = Math.min(99, (currentLength[0] * 100) / targetLength[0]);
                                    sendEvent(emitter, "progress", Map.of(
                                            "content", toSend,
                                            "progress", progress,
                                            "current", currentLength[0],
                                            "total", targetLength[0]
                                    ));
                                } catch (IllegalStateException e) {
                                    log.debug("SSE连接已关闭，停止发送");
                                } catch (Exception e) {
                        ...
                        // 流结束时，标记完成状态，但由等待循环统一发送剩余内容
                        if (isDone) {
                            isStreamComplete[0] = true;
                            log.info("章节流式生成完成标记，总字数：{}", currentLength[0]);

                            // 记录AI交互日志（成功）
                            try {
                                long duration = System.currentTimeMillis() - finalStartTime;
                                String requestParams = String.format("{\"temperature\":%s,\"maxTokens\":%s}",
                                    finalRequest.getTemperature(), finalRequest.getMaxTokens());
                                ...
                                aiInteractionLogService.logInteraction(
                                    ...
                                    provider.getModel(), // 模型名称
                                    finalPrompt, // 请求提示词
                                    requestParams, // 请求参数
                                    contentBuilder.toString(), // 响应内容
                                    currentLength[0], // 响应token数（使用字数近似）
                                    duration, // 响应耗时
                                    true, // 成功
                                    null // 错误信息
                                );
                            } catch (Exception e) {
                                log.error("记录AI交互日志失败: {}", e.getMessage(), e);
                            }

                            // 注意：缓冲区剩余内容由等待循环统一发送，确保前端能接收
                        }
                    }

                    @Override
                    public void onError(String error) {
                        log.error("章节流式生成失败：{}", error);
                        ...
                        try {
                            ...
                            log.error("记录AI交互错误日志失败: {}", e.getMessage(), e);
                        }

                        try {
                            sendError(emitter, "生成失败: " + error);
                        } catch (Exception e) {
                ...
                // 等待流式生成完成（最多等待10分钟）
                // 判断标准：isStreamComplete=true 且最近8秒内没有新token（给AI更多思考时间）
                int maxWaitSeconds = 600;
                int waitedSeconds = 0;
                final long TOKEN_TIMEOUT_MS = 8000; // 8秒内没有新token认为完成（避免误判）
                final long PROGRESS_TIMEOUT_MS = 30000; // 30秒没有任何进度认为卡住
                ...
                while (waitedSeconds < maxWaitSeconds) {
                    try {
                        ...
                        long timeSinceLastToken = System.currentTimeMillis() - lastTokenTime[0];
                        long timeSinceLastProgress = System.currentTimeMillis() - lastProgressTime;

                        // 检查是否有新进度（内容长度增加）
                        if (currentLength[0] > lastContentLength) {
                            lastProgressTime = System.currentTimeMillis();
                            lastContentLength = currentLength[0];
                        }

                        // 完成判断：流已结束且超过TOKEN_TIMEOUT没有新token
                        boolean isComplete = isStreamComplete[0] && timeSinceLastToken > TOKEN_TIMEOUT_MS;
                        // 卡住判断：30秒没有任何新内容（可能是AI卡住了）
                        boolean isStuck = !isStreamComplete[0] && timeSinceLastProgress > PROGRESS_TIMEOUT_MS;

                        if (isComplete) {
                            log.info("流式生成完成，总字数：{}，总耗时：{}秒", currentLength[0], waitedSeconds);
                            break;
                        }

                        if (isStuck) {
                            log.warn("AI生成似乎卡住了，{}秒内没有新内容，当前字数：{}",
                                    PROGRESS_TIMEOUT_MS / 1000, currentLength[0]);
                            // 如果已经有内容，尝试继续；如果完全没内容，报错
                            if (currentLength[0] > 100) {
                                log.info("已有足够内容，继续处理");
                                break;
                            } else {
                                throw new RuntimeException("AI生成卡住，" + PROGRESS_TIMEOUT_MS / 1000 + "秒内没有新内容");
                            }
                        }

                        if (waitedSeconds % 10 == 0 && currentLength[0] > 0) {
                            log.info("等待流完成... 已接收{}字，已等待{}秒，最后token距今{}秒",
                                    currentLength[0], waitedSeconds, timeSinceLastToken / 1000);
                        }
                ...
                if (buffer.length() > 0) {
                    try {
                        log.info("发送缓冲区剩余内容，长度：{}", buffer.length());
                        var toSend = buffer.toString();
                        buffer.setLength(0);
                        ...
                        log.error("发送缓冲区剩余内容失败", e);
                    }
                }

                // 处理最终内容
                var content = processFormatMarkers(contentBuilder.toString());

                log.info("流式生成检查：isStreamComplete={}, contentLength={}",
                        isStreamComplete[0], content.length());

                if (!isStreamComplete[0] || content.isEmpty()) {
                    log.warn("流式生成未完成或内容为空，跳过保存");
                    try {
                        sendError(emitter, "生成失败：未生成任何内容");
                    } catch (Exception e) {
                        log.error("发送错误失败", e);
                    }
                    return;
                ...
                log.info("开始保存章节到数据库，字数：{}", content.length());

                // 9. 更新章节最终内容（章节已在开始时创建）
                chapter.setTitle(plan.getChapterTitle());
                chapter.setContent(content);
                chapter.setWordCount(content.length());
                chapter.setUpdateTime(LocalDateTime.now());
                chapterMapper.updateById(chapter);
                log.info("更新章节内容成功，chapterId={}", chapter.getId());

                // 10. 同步生成剧情概要和保存剧情记忆（必须成功，影响后续章节质量）
                String plotSummary = null;
                try {
                    // 10.1 生成并保存剧情概要
                    plotSummary = generatePlotSummary(projectId, plan, chapter, content, plan.getChapterTitle(), plan.getChapterNumber());
                    chapter.setPlotSummary(plotSummary);
                    chapterMapper.updateById(chapter);
                    log.info("同步生成剧情概要成功，chapterId={}", chapter.getId());

                    // 10.2 保存详细剧情记忆（用于后续章节的连贯性）
                    saveChapterPlotMemory(projectId, chapter, plan, content, plotSummary);
                    log.info("同步保存剧情记忆成功，chapterId={}", chapter.getId());

                } catch (Exception e) {
                    log.error("保存剧情记忆失败，这会影响后续章节生成质量！chapterId={}", chapter.getId(), e);
                ...
                emitter.complete();
                log.info("章节生成完成，planId={}, chapterId={}", planId, chapter.getId());

            } catch (Exception e) {
                log.error("生成章节失败，planId={}", planId, e);
