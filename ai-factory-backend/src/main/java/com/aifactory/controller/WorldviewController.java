...
public class WorldviewController {
     ...
     */
    @Operation(
            summary = "AI同步生成世界观设定（已弃用）",
            description = "使用AI同步生成世界观设定。此接口会阻塞等待AI生成完成，建议使用异步接口 /generate-async 替代。生成的内容包括世界类型、背景描述、力量体系、地理环境、势力分布、时间线和世界规则"
    )
    @ApiResponses(value = {
            @ApiResponse(responseCode = "200", description = "生成成功，返回世界观设定",
                    content = @Content(schema = @Schema(example = "{\"worldview\": {...}, \"message\": \"世界观生成成功\"}"))),
            @ApiResponse(responseCode = "404", description = "项目不存在"),
            @ApiResponse(responseCode = "500", description = "AI生成失败")
    })
    @PostMapping("/generate")
    public Result<Map<String, Object>> generateWorldview(
            @Parameter(description = "项目ID", required = true, example = "1")
            @PathVariable Long projectId,
            @io.swagger.v3.oas.annotations.parameters.RequestBody(
                    description = "请求参数（可选，系统会自动从项目配置获取所需参数）",
        ...
        try {
            log.info("开始AI生成世界观设定，projectId={}", projectId);
            ...
            // 6. 解析并保存世界观
            NovelWorldview worldview = parseAndSaveWorldview(projectId, responseContent, storyGenre);

            if (worldview != null) {
                log.info("世界观设定生成完成，ID: {}", worldview.getId());

                Map<String, Object> result = new HashMap<>();
                result.put("worldview", worldview);
                result.put("message", "世界观生成成功");

                return Result.ok(result);
            } else {
                return Result.error("解析世界观失败，请重试");
            }

        } catch (Exception e) {
            log.error("AI生成世界观设定失败，projectId={}", projectId, e);
            return Result.error("生成世界观设定失败：" + e.getMessage());
        }
    }
