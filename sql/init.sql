...
INSERT INTO `ai_prompt_template` (`id`, `template_code`, `template_name`, `service_type`, `scenario`, `current_version_id`, `description`, `tags`, `is_active`, `is_system`, `created_by`, `created_time`, `updated_by`, `updated_time`) VALUES (6, 'llm_outline_chapter_generate', '章节规划生成模板', 'llm', 'outline_chapter_generate', 20, '用于生成分卷章节规划的提示词模板，包含专业网文结构设计指导', '小说,大纲,章节,生成', 1, 1, NULL, '2026-02-06 14:21:54', NULL, '2026-02-26 18:39:53');
...
UPDATE `ai_prompt_template_version` SET `is_active` = 0 WHERE `id` = 15 AND `template_id` = 15;

INSERT INTO `ai_prompt_template_version` (`id`, `template_id`, `version_number`, `template_content`, `variable_definitions`, `version_comment`, `is_active`, `created_by`, `created_time`) VALUES (19, 15, 2, '# 角色提取任务\r\n\r\n你是一位专业的小说角色分析师。请从以下章节内容中提取所有出现的人物角色信息。\r\n\r\n## 提取要求\r\n1. 识别章节中所有有名字的角色\r\n2. 推断角色的类型（protagonist/supporting/antagonist/npc）\r\n3. 推断角色的性别和年龄（如果有相关信息）\r\n4. 提取角色的性格特点和外貌特征\r\n5. 描述角色在本章中的状态变化\r\n6. 如果涉及修炼体系，提取角色的修为等级信息\r\n\r\n## 角色类型定义（严格遵循）\r\n- protagonist：主角，第一视角人物，故事核心人物。一个故事通常只有1-2个主角\r\n- supporting：重要配角，多次出现、对剧情有推动作用\r\n- antagonist：反派，与主角对抗、制造冲突的人物\r\n- npc：过场人物，只在一两章出现、一两句台词的边缘角色\r\n\r\n{existingRoleDistribution}\r\n\r\n## 输出格式\r\n请使用XML格式输出，标签使用单字母以节省token：\r\n\r\n```xml\r\n<M>\r\n  <C>\r\n    <N>角色名称</N>\r\n    <T>角色类型(protagonist/supporting/antagonist/npc)</T>\r\n    <G>性别(male/female/other)</G>\r\n    <A>年龄</A>\r\n    <S>本章状态变化</S>\r\n    <P>性格特点（逗号分隔）</P>\r\n    <F>外貌特征描述</F>\r\n    <V>\r\n      <SYS>修炼体系名称</SYS>\r\n      <LV>当前境界等级</LV>\r\n      <CH>本章境界变化</CH>\r\n    </V>\r\n    <AP>外貌/装扮变化</AP>\r\n    <PR>性格展现</PR>\r\n    <AB>能力展现</AB>\r\n    <CD>角色成长</CD>\r\n    <DG>核心对话摘要</DG>\r\n  </C>\r\n</M>\r\n```\r\n\r\n## 注意事项\r\n1. 每个角色使用一个C标签包裹\r\n2. 如果角色有多个修炼体系，使用多个V标签\r\n3. 如果章节中没有明确提到某项信息，对应标签可以省略\r\n4. 不要虚构不存在的角色信息\r\n\r\n## 势力关联\r\n如章节中出现角色所属势力/门派/组织，为每个角色添加FC标签：\r\n<FC><N>势力名称</N><R>职位/角色</R></FC>\r\n角色可属于多个势力（转投、双重身份等），每个势力一个FC标签。势力名称必须使用已有势力列表中的名称。\r\n\r\n{chapterInfo}', '[{\"desc\":\"章节号\",\"name\":\"chapterNumber\",\"type\":\"integer\",\"required\":true},{\"desc\":\"章节标题\",\"name\":\"chapterTitle\",\"type\":\"string\",\"required\":true},{\"desc\":\"章节内容\",\"name\":\"chapterContent\",\"type\":\"string\",\"required\":true},{\"desc\":\"修炼体系约束\",\"name\":\"powerSystemConstraint\",\"type\":\"string\",\"required\":false},{\"desc\":\"已有角色参考\",\"name\":\"existingCharacters\",\"type\":\"string\",\"required\":false},{\"desc\":\"已有角色类型分布\",\"name\":\"existingRoleDistribution\",\"type\":\"string\",\"required\":false}]', 'v2: 增加中文角色类型定义、FC势力标签、已有角色分布注入', 1, NULL, '2026-04-06 00:00:00');

UPDATE `ai_prompt_template` SET `current_version_id` = 19 WHERE `id` = 15;

-- Deactivate character extract template v2 and activate v3 (RL/SL split format)
INSERT INTO `ai_prompt_template_version` (`id`, `template_id`, `version_number`, `template_content`, `variable_definitions`, `version_comment`, `is_active`, `created_by`, `created_time`) VALUES (21, 15, 3, '# 角色提取任务\r\n\r\n你是一位专业的小说角色分析师。请从以下章节内容中提取所有出现的人物角色信息。\r\n\r\n## 提取要求\r\n1. 识别章节中所有有名字的角色\r\n2. 推断角色的类型（protagonist/supporting/antagonist/npc）\r\n3. 推断角色的性别和年龄（如果有相关信息）\r\n4. 提取角色的性格特点和外貌特征\r\n5. 描述角色在本章中的状态变化\r\n6. 如果涉及修炼体系，提取角色的修为等级信息\r\n\r\n## 角色类型定义（严格遵循）\r\n- protagonist：主角，第一视角人物，故事核心人物。一个故事通常只有1-2个主角\r\n- supporting：重要配角，多次出现、对剧情有推动作用\r\n- antagonist：反派，与主角对抗、制造冲突的人物\r\n- npc：过场人物，只在一两章出现、一两句台词的边缘角色\r\n\r\n{existingRoleDistribution}\r\n\r\n## 输出格式\r\n请使用XML格式输出，标签使用单字母以节省token：\r\n\r\n```xml\r\n<M>\r\n  <C>\r\n    <N>角色名称</N>\r\n    <T>角色类型(protagonist/supporting/antagonist/npc)</T>\r\n    <G>性别(male/female/other)</G>\r\n    <A>年龄</A>\r\n    <S>本章状态变化</S>\r\n    <P>性格特点（逗号分隔）</P>\r\n    <F>外貌特征描述</F>\r\n    <V>\r\n      <SYS>修炼体系名称</SYS>\r\n      <RL>大境界名称</RL>\r\n      <SL>小境界名称</SL>\r\n      <CH>本章境界变化</CH>\r\n    </V>\r\n    <AP>外貌/装扮变化</AP>\r\n    <PR>性格展现</PR>\r\n    <AB>能力展现</AB>\r\n    <CD>角色成长</CD>\r\n    <DG>核心对话摘要</DG>\r\n  </C>\r\n</M>\r\n```\r\n\r\n## 修炼体系输出约束（极其重要）\r\n- <SYS>必须是上述修炼体系的名称（如"古神道"）\r\n- <RL>必须是大境界名称（如"醒血境"），必须与等级划分中的大境界完全一致\r\n- <SL>必须是小境界名称（如"感血"），必须与括号中的小境界完全一致\r\n- 如果不确定具体小境界，<SL>可以省略，但<RL>必须准确\r\n- 禁止编造不存在的修炼体系、大境界或小境界名称\r\n\r\n## 注意事项\r\n1. 每个角色使用一个C标签包裹\r\n2. 如果角色有多个修炼体系，使用多个V标签\r\n3. 如果章节中没有明确提到某项信息，对应标签可以省略\r\n4. 不要虚构不存在的角色信息\r\n\r\n## 势力关联\r\n如章节中出现角色所属势力/门派/组织，为每个角色添加FC标签：\r\n<FC><N>势力名称</N><R>职位/角色</R></FC>\r\n角色可属于多个势力（转投、双重身份等），每个势力一个FC标签。势力名称必须使用已有势力列表中的名称。\r\n\r\n{chapterInfo}', '[{\"desc\":\"章节号\",\"name\":\"chapterNumber\",\"type\":\"integer\",\"required\":true},{\"desc\":\"章节标题\",\"name\":\"chapterTitle\",\"type\":\"string\",\"required\":true},{\"desc\":\"章节内容\",\"name\":\"chapterContent\",\"type\":\"string\",\"required\":true},{\"desc\":\"修炼体系约束\",\"name\":\"powerSystemConstraint\",\"type\":\"string\",\"required\":false},{\"desc\":\"已有角色参考\",\"name\":\"existingCharacters\",\"type\":\"string\",\"required\":false},{\"desc\":\"已有角色类型分布\",\"name\":\"existingRoleDistribution\",\"type\":\"string\",\"required\":false},{\"desc\":\"大境界名称\",\"name\":\"realmLevel\",\"type\":\"string\",\"required\":false},{\"desc\":\"小境界名称\",\"name\":\"subLevel\",\"type\":\"string\",\"required\":false}]', 'v3: 拆分LV为RL/SL标签，增加修炼体系名称严格匹配约束', 1, NULL, '2026-04-08 00:00:00');
...
-- Phase 12: Chapter planning template v2 with character planning XML tags
INSERT INTO `ai_prompt_template_version` (`id`, `template_id`, `version_number`, `template_content`, `variable_definitions`, `version_comment`, `is_active`, `created_by`, `created_time`) VALUES (20, 6, 2, '你是一位资深的网络小说作家和编辑，精通章节节奏设计和叙事技巧。\n\n请为第{volumeNumber}卷规划详细章节大纲。\n\n## 核心创作要求\n\n请严格遵循以下创作设置进行章节规划：\n\n### 叙事与写作设置\n{narrativeSettings}\n\n---\n\n## 小说基础信息\n\n【故事背景】\n{projectDescription}\n\n【故事基调】{storyTone}\n【故事类型】{storyGenre}\n\n---\n\n## 分卷信息\n{volumeInfo}\n\n---\n\n## 世界观设定\n{worldviewInfo}\n\n---\n\n## 登场角色\n{characterInfo}\n\n---\n\n## 角色规划要求（重要）\n\n每个章节必须规划登场角色列表。对于每个登场角色，说明其在本章的戏份和重要程度。\n你可以使用角色列表中的已有角色，也可以根据情节需要安排新角色登场。\n\n---\n\n## 章节规划要求\n\n【预计章节数】{chapterCount}章\n【每章字数】{wordCount}字左右\n\n{paceGuide}\n\n---\n\n## 专业网文结构设计 - 必须遵循\n\n**1. 三明一暗伏笔架构**\n- 明线1（主线）：主角成长、升级、变强\n- 明线2（情感线）：友情、爱情、师徒情发展\n- 明线3（任务线）：寻宝、探秘、解决事件\n- 暗线（核心秘密）：贯穿全卷甚至全书的最大悬念\n\n**2. 章节节奏设计（每3章一个小循环）**\n- 第1章（铺垫章）：铺设场景、埋下伏笔、制造期待\n- 第2章（升级章）：冲突升级、遇到阻碍、紧张感增强\n- 第3章（高潮章）：小高潮爆发、问题部分解决、但留下更大悬念\n\n**3. 关键事件设计原则（极重要！）**\n- 每章最多1-2个关键事件，强烈建议只有1个核心关键事件\n- 关键事件过多样会导致后续章节生成时内容分散、情节展开不够充分、字数不足\n- 关键事件应该是本章最核心、最推动剧情发展的那件事\n- 细节描写、日常对话、场景转换、配角小动作等不属于关键事件\n- 示例分析：\n  ✅ 正确：\"主角首次进入秘境探索\"（1个，核心明确，适合后续展开）\n  ❌ 错误：\"主角遇到守门人；守门人检查身份；主角进入秘境；发现一片森林\"（4个，过于细碎，后续无法充分展开）\n- 记住：关键事件是骨架，细节描写要留白给后续章节生成时填充\n\n**4. 悬念设置要求**\n- 每章结尾必须设置钩子（让读者想读下一章）\n- 每3章一个小高潮，满足读者期待\n- 每章揭示一个新信息，但引出更多问题\n- 长期悬念要逐步透露线索，保持神秘感\n\n**5. 伏笔埋设原则**\n- 短期伏笔（1-3章回收）：小道具、小线索、人物小动作\n- 中期伏笔（5-10章回收）：次要角色的秘密、次要物品的来历\n- 长期伏笔（本卷或全书回收）：核心秘密、主角身世、世界真相\n- 回收伏笔时要有情感冲击，让读者有\"原来如此\"的感觉\n\n**6. 章节起终点设计（重要！确保剧情连贯）**\n- 每章必须有明确的起点场景和终点场景\n- **起点场景**：描述章节开始时主角在哪里、什么时间、什么状态\n- **终点场景**：描述章节结束时主角在哪里、什么时间、什么状态\n- **章节衔接**：第N章的终点场景 = 第N+1章的起点场景\n- **防止时间倒流**：明确标注场景状态，避免剧情重复或倒退\n\n---\n\n## XML输出格式\n\n【重要】请严格按照以下XML格式返回数据（使用小写单字母标签）：\n<c>\n  <o>\n    <n>1</n>\n    <v>{volumeNumber}</v>\n    <t><![CDATA[章节标题]]></t>\n    <p><![CDATA[本章情节大纲（100-150字）]]></p>\n    <e><![CDATA[本章核心关键事件（最多1-2个，建议1个）]]></e>\n    <g><![CDATA[本章目标：主角在本章要达成什么]]></g>\n    <w>{wordCount}</w>\n    <s><![CDATA[章节起点场景（地点、时间、状态）]]></s>\n    <f><![CDATA[章节终点场景（地点、时间、状态）]]></f>\n    <ch>\n      <cn><![CDATA[角色名称]]></cn>\n      <cd><![CDATA[该角色在本章的戏份梗概（50-100字）]]></cd>\n      <ci>high</ci>\n    </ch>\n    <ch>\n      <cn><![CDATA[另一个角色名称]]></cn>\n      <cd><![CDATA[该角色在本章的戏份梗概]]></cd>\n      <ci>medium</ci>\n    </ch>\n  </o>\n</c>\n\n### 标签说明\n- c: chapters（章节列表根元素）\n- o: chapter（单个章节，one chapter）\n- n: number（章节序号）\n- v: volume（所属卷号）\n- t: title（章节标题）\n- p: plot（情节大纲，100-150字）\n- e: events（关键事件，最多1-2个）\n- g: goal（章节目标）\n- w: words（目标字数）\n- s: start（起点场景）\n- f: finish（终点场景）\n- ch: character（规划角色，每个章节可包含0-3个）\n- cn: character name（角色名称）\n- cd: character description（该角色在本章的戏份梗概）\n- ci: character importance（重要程度：high/medium/low）\n\n### 格式要求\n1. 直接返回纯XML格式，不要使用markdown代码块标记（不要使用```xml和```）\n2. 不要包含任何解释文字、注释或说明\n3. 只返回XML数据本身\n4. 对于长文本内容（可能包含特殊字符），请使用CDATA标签包裹：<![CDATA[内容]]>\n5. p标签的情节大纲需达到100-150字，概述本章的主要内容和走向\n6. s和f标签的场景描述要具体，包含地点、时间、人物状态\n7. <ch>角色规划标签为可选标签，过渡章节可以不包含角色规划\n8. ci标签只能使用high/medium/low三个值\n\n现在请返回{chapterCount}个章节的完整XML数据（纯XML，不要markdown标记）：', '[{\"desc\":\"卷号\",\"name\":\"volumeNumber\",\"type\":\"integer\",\"required\":true},{\"desc\":\"故事背景\",\"name\":\"projectDescription\",\"type\":\"string\",\"required\":true},{\"desc\":\"故事基调\",\"name\":\"storyTone\",\"type\":\"string\",\"required\":true},{\"desc\":\"故事类型\",\"name\":\"storyGenre\",\"type\":\"string\",\"required\":true},{\"desc\":\"叙事与写作设置\",\"name\":\"narrativeSettings\",\"type\":\"string\",\"required\":false},{\"desc\":\"分卷信息\",\"name\":\"volumeInfo\",\"type\":\"string\",\"required\":true},{\"desc\":\"世界观设定\",\"name\":\"worldviewInfo\",\"type\":\"string\",\"required\":false},{\"desc\":\"登场角色\",\"name\":\"characterInfo\",\"type\":\"string\",\"required\":false},{\"desc\":\"章节数\",\"name\":\"chapterCount\",\"type\":\"integer\",\"required\":true},{\"desc\":\"每章字数\",\"name\":\"wordCount\",\"type\":\"integer\",\"required\":true},{\"desc\":\"章节节奏指导\",\"name\":\"paceGuide\",\"type\":\"string\",\"required\":false}]', 'v2: add character planning output instructions and XML tags (Phase 12)', 1, NULL, NOW());

-- Deactivate old chapter planning template version
UPDATE `ai_prompt_template_version` SET `is_active` = 0 WHERE `id` = 6 AND `template_id` = 6;

-- Update chapter planning template to use new version
UPDATE `ai_prompt_template` SET `current_version_id` = 20, `updated_time` = NOW() WHERE `id` = 6;
